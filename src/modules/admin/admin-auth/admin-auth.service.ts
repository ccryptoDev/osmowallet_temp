...
export class AdminAuthService {
    ...
    constructor(
        ...
        const config = new Config(this.configService.getOrThrow('CORBADO_PROJECT_ID'), this.configService.getOrThrow('CORBADO_API_SECRET'));
        config.setFrontendAPI(this.configService.getOrThrow('CORBADO_FRONTEND_API'));
    ...
    async signinWithEmailAndPassword(data: AdminAuthDto) {
        if (!data.email.endsWith('@osmowallet.com')) throw new UnauthorizedException();
        await this.validateApp(data);
        ...
        if (!user) throw new BadRequestException('Credenciales incorrectas');
        const passwordMatches = await this.verifyPassword(user.password, data.password);

        if (!passwordMatches) throw new BadRequestException('Credenciales incorrectas');
        const tokens = await this.getTokens(user.id);
        return tokens;
    }
