...
export class LiquiditySell extends LiquidityTransaction implements AdminTransaction {
    ...
    async create(): Promise<void> {
        ...
        await this.manager.transaction('SERIALIZABLE', async (entityManager) => {
            ...
            if (!wallet) throw new BadRequestException('Invalid coin wallet');
            ...
            await this.createAndInsertTransaction(
                ...
                TransactionSubtype.CREDIT_FIAT_SELL,
                transactionGroup.id,
                entityManager,
            );
            const cryptoSubtype: TransactionSubtype =
                cryptoWallet.coin.acronym == CoinEnum.SATS ? TransactionSubtype.DEBIT_BTC_SELL : TransactionSubtype.DEBIT_STABLE_OSMO;

            await this.createAndInsertTransaction(
                cryptoWallet,
                amountCryptoToDebit.toNumber(),
