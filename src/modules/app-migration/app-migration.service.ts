...
export class AppMigrationService {
    ...
    private async getUserBalances(rehiveUser: RehiveUser, ibexAccount: RehiveUserIbexAccount): Promise<UserBalance> {
        if (!ibexAccount) {
        ...
        try {
            ...
            const ibexAccountDetails = await this.ibexService.getAccountDetails(ibexAccount.accountId);
            let satsBalance = 0;
    ...
    async getRecurrentBuys(rehiveUser: RehiveUser): Promise<RehiveRecurrentBuy[]> {
        const recurrentBuy = await admin.firestore().collection('RecurrentBuys').where('userId', '==', rehiveUser.id).get();
    ...
    async getPreferences(rehiveUser: RehiveUser): Promise<RehiveUserPreference | undefined> {
        const preferences = await admin.firestore().collection('Preferences').doc(rehiveUser.id).get();
    ...
    async getIbexAccount(rehiveUser: RehiveUser): Promise<RehiveUserIbexAccount> {
        const ibexAccount = await admin.firestore().collection('IbexAccounts').where('id', '==', rehiveUser.id).get();
        if (ibexAccount.docs.length > 0) {
            ...
            await admin.firestore().collection('IbexAccounts').add({
    ...
    async getVerification(rehiveUser: RehiveUser) {
        const response = await admin.firestore().collection('Metamap').where('user_id', '==', rehiveUser.id).get();
    ...
    async creteUserData(
        ...
        const fundingFeature = this.features.find((feature) => feature.name == FeatureEnum.FUNDING);
        const withdrawFeature = this.features.find((feature) => feature.name == FeatureEnum.WITHDRAW);
        ...
        try {
            await this.userRepository.manager.transaction(async (entityManager) => {
                ...
                const fundingTransactionLimits = entityManager.create(UserTransactionLimit, { user: user, feature: fundingFeature });
                const withdrawTransactionLimits = entityManager.create(UserTransactionLimit, { user: user, feature: withdrawFeature });
                await entityManager.insert(UserTransactionLimit, [fundingTransactionLimits, withdrawTransactionLimits]);
    ...
    async createRecurrentBuys(user: User, rehiveRecurrentBuys: RehiveRecurrentBuy[]) {
    ...
    async createVerification(verification: any, user: User) {
        try {
            if (verification && verification.verification_id != null) {
                ...
                const kycVerificationSteps: KycVerificationStep[] = this.kycSteps.map((step) => {
    ...
    async getUserDataMigration(rehiveUser: RehiveUser | undefined) {
        ...
        if (ibexAccount != null) {
            ...
            this.creteUserData(rehiveUser, userBalance, ibexAccount, verification, recurrentBuys, preferences);
        }
    }
    ...
    async runMigration() {
        ...
        const [features, coins, role, periods, tier, usernames] = await Promise.all([
        ...
        await Promise.all([this.getRehiveUserBalances(), this.getRehiveUsers()]);
        let usersFixed = this.users.map((user: any) => {
            if (user.id !== undefined && user.email !== undefined && user.mobile !== undefined) {
                return {
                    id: user.id,
    ...
    async getRehiveUsers() {
        ...
        let data = await axios.get(url, { headers: headers }).then((response) => response.data.data);
