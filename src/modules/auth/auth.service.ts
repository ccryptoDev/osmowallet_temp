...
export class AuthService {
    ...
    async sendEmailVerification(authUser: AuthUser, otpType: OTP = OTP.VERIFICATION) {
        try {
            ...
            const template = new OTPSigninTemplate([{ email: user.email, name: user.username }], otp);
            this.sengridService.sendMail(template);
        } catch (error) {
            throw error;
        }
    ...
    async signUp(signUpDto: SignUpDto): Promise<any> {
        ...
        if (signUpDto.mobile != undefined) {
            ...
            if (mobileExists) {
                throw new BadRequestException('Ya existe un usuario con este número de teléfono');
            }
        }
        ...
        await this.userRepository.manager.transaction(async (transactionalEntityManager) => {
            ...
            const withdrawTransactionLimits = transactionalEntityManager.create(UserTransactionLimit, {
                user: newUser,
                feature: withdrawFeature,
            });
            await transactionalEntityManager.insert(UserTransactionLimit, [withdrawTransactionLimits]);
    ...
    async verifyAuthOTP(data: AuthOTPDto) {
        try {
            ...
            if (ibexAccount) {
                ...
                this.googleCloudTaskService.createInternalTask(this.CREATE_IBEX_ADDRESS_QUEUE, body, this.CREATE_IBEX_ADDRESS_URL);
            }
            return tokens;
        } catch (error) {
            throw error;
    ...
    async storeSession(authUser: AuthUser, data: SessionDto, refreshToken: string) {
        const user = await this.userRepository.findOneBy({ id: authUser.sub });
        if (!user) throw new NotFoundException('User not found');
