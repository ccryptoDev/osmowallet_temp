...
export class AccountingReportHelper {
    static async writeWalletsToExcel(wallets: WalletBalance[]) {
        ...
        groupedWallets.map((walletGroup) =>
            AccountingReportHelper.writeWorksheet(walletGroup.wallets, workbook, `UserWallets (${walletGroup.currency})`),
        );
    ...
    static async generateReportTemplate(wallets: WalletBalance[]): Promise<SendgridTemplate> {
        ...
        const template = new MonthlyReportTemplate(
            [{ email: 'as@singularagency.co', name: 'mika' }],
            [
                {
