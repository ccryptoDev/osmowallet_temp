...
export class JsonBillAdapter {
    ...
    static createBillTemplate(payload: BillPayload) : InfilePayload {
        ...
        const billId = 'FACT-' + currentDate.getMonth().toString().padStart(2,'0') + currentDate.getFullYear() + '-' + payload.userId + '-' + payload.currency
    ...
    private static buildXml(item: any, payload: BillPayload, date: string) {
        ...
        templateCopy['dte:GTDocumento']['dte:SAT']['dte:DTE']['dte:DatosEmision']['dte:Emisor']['@NITEmisor'] = process.env.INFILE_NIT_EMISOR;
        ...
        templateCopy['dte:GTDocumento']['dte:SAT']['dte:DTE']['dte:DatosEmision']['dte:Totales']['dte:TotalImpuestos']['dte:TotalImpuesto']['@TotalMontoImpuesto'] = iva
        return templateCopy;
    }
    ...
    private static getTime(currentDate: Date) {
        ...
        var milli = currentDate.getMilliseconds().toString().padStart(3, '0')
