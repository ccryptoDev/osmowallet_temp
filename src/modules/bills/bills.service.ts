    ...
    async createUserBill(data: CreateBillDto) {
        try {
            ...
            const totals = await this.transactionRepository
                ...
                .select([
                    "CONCAT(fromUser.firstName, ' ', fromUser.lastName) as fullName",
                    'fromUser.id as userId',
                    'fromUser.email as email',
                    'coin.acronym as acronym',
                ...
                .andWhere('transaction.subtype IN (:...names)', {
                    names: ['fee-buy', 'fee-sell', 'fee-autoconvert-sell', 'fee-withdraw', 'fee-funding'],
                })
    ...
    async addBillToQueue(bill: InfilePayload) {
        const payload: ExternalTask = {
            ...
            headers: {
                ...
                Identificador:
                    bill.billId + new Date().toLocaleDateString('default', { month: '2-digit', year: 'numeric' }).replace('/', '-'),
    ...
    async getUsersToBill() {
        ...
        const users = await this.transactionGroupRepository
            ...
            .where('transaction.subtype IN (:...names)', {
                names: ['fee-buy', 'fee-sell', 'fee-recurrent-buy', 'fee-autoconvert-sell', 'fee-withdraw', 'fee-funding'],
            })
