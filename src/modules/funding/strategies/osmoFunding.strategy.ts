...
export class OsmoBankFunding extends DynamicDtoValidator implements Funding {
    ...
    async notify(transactionGroup: TransactionGroup) {
        const date = new Date();
        const template = new FundingPendingTemplate([{ email: this.user.email, name: this.user.firstName }], {
            amount: this.body.amount,
        ...
        const osmoTemplate = new FundingPendingOsmoTemplate(emails, this.user.firstName + ' ' + this.user.lastName, this.user.email, {
            amount: this.body.amount,
            currency: this.coin.acronym,
            date: formatDateToSpanish(date),
            status: Status.PENDING,
