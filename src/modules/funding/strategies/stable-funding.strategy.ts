...
export class StableFunding implements Funding {
    ...
    async fund(): Promise<any> {
        ...
        await this.manager.transaction('SERIALIZABLE', async (transactionalEntityManager) => {
            ...
            if (!userWallet || !osmoWallet || !osmoFeeWallet)
    ...
    private notify(transactionGroup: TransactionGroup, user: User, amount: number) {
        const emails =
            process.env.ENV == 'PROD'
                ? [
                      { email: 'victor@osmowallet.com', name: 'Victor' },
                      { email: 'piero@osmowallet.com', name: 'Piero' },
                  ]
                : [{ email: 'as@singularagency.co', name: 'Piero' }];
        const date = new Date();
        const template = new FundingPendingOsmoTemplate(emails, user.firstName + ' ' + user.lastName, user.email, {
            amount: amount,
        ...
        const confirmationUserTemplate = new FundingPendingTemplate([{ email: user.email, name: user.firstName }], {
            amount: amount,
            currency: this.coin.acronym,
            date: formatDateToSpanish(date),
            status: Status.PENDING,
    ...
    private async getProofData(
        file: Express.Multer.File,
        user: User,
    ): Promise<{ proofUrl: string; proofPath: string; proofUrlExpiry: number }> {
    ...
    private async setDataToUse() {
        const [stableCoin, settings] = await Promise.all([this.manager.findOneBy(Coin, { acronym: 'USDT' }), this.manager.find(Setting)]);
    ...
    private async validateInputData() {
        ...
        const usdAmount = exchangeCoinToUSD(this.body.amount, coin);
