...
export class IbexService {
    ...
    private async getToken(): Promise<string> {
        ...
        if (!ibexAccessToken) {
            throw new BadRequestException('Ibex access token not found');
        }
    ...
    async createUserIbexAccount(userId: string) {
        ...
        this.googleTaskService.createInternalTask(this.CREATE_IBEX_ADDRESS_QUEUE, body, this.CREATE_IBEX_ADDRESS_URL);
        const usernameBody: CreateIbexUsernameDto = {
            accountId: ibexAccount.id,
            ibexAccountId: ibexAccount.account,
            userId: userId,
        };
        this.googleTaskService.createInternalTask(this.CREATE_USERNAME_ADDRESS_QUEUE, usernameBody, this.CREATE_USERNAME_ADDRESS_URL);
    }
    ...
    async generateInvoiceForStrike(
        accountId: string,
        amountSats: number,
        userId: string,
        referenceId: string,
    ): Promise<IbexLightningTransaction> {
        try {
            const ibexAccessToken = await this.getToken();
    ...
    async createAddresses(accountId: string) {
        ...
        try {
            ...
            await this.addressesModel.updateOne(
                { user: ibexAccount.user.id },
                {
                    $set: {
                        addresses: addresses,
