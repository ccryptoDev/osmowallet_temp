...
export class MeService {
    ...
    async updateUsername(authUser: AuthUser, data: UpdateUsernameDto) {
        try {
            ...
            if (user.usernameChanges == 2) throw new BadRequestException('Username has been changed twice');
    ...
    async getRecentContacts(authUser: AuthUser) {
        try {
            ...
            const usersIds = recentContactRecords.map((recentContactRecord) => recentContactRecord.contact.id);
    ...
    async updateProfilePicture(authUser: AuthUser, file: Express.Multer.File, data: ProfilePictureDto) {
        try {
            const user = await this.userRepository.findOneBy({ id: authUser.sub });
            if (!user) throw new BadRequestException('No se encontro ningun usuario con este id');
            ...
            throw new InternalServerErrorException('Error interno al actualizar foto de perfil');
        }
    }
    ...
    async createBankAccount(authUser: AuthUser, data: BankAccountDto) {
        try {
            ...
            if (bank == null) {
                throw new BadRequestException('No se encontro ninguna banco con este id');
            }
            const user = await this.userRepository.findOneBy({ id: authUser.sub });
            if (!user) {
            ...
            if (!['USD', 'GTQ'].includes(coin.acronym)) throw new BadRequestException('Invalid coin');
            ...
            if (existsAccount.length > 0) throw new BadRequestException('Ya existe una cuenta banco creada con estos mismos datos');
    ...
    async updateBankAccount(authUser: AuthUser, data: BankAccountDto, bankId: string) {
        try {
            const bank = await this.bankRepository.findOneBy({ id: data.bankId });
            ...
            if (bankAccount === null) throw new BadRequestException('Invalid bankAccount');
    ...
    async deleteBankAccount(authUser: AuthUser, bankId: string) {
        try {
            ...
            if (!bankAccount) throw new NotFoundException('Este banco no exite en tu cuenta');
    ...
    async updatePreference(authUser: AuthUser, data: PreferenceDto) {
        ...
        if (!preferenceRecord) throw new BadRequestException('No se encontro ninguna preferencia');
