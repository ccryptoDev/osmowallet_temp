...
export class OsmoWalletUserBank implements PartnerFlowStrategy {
    ...
    async deposit(): Promise<PartnerStatus> {
        const osmoUserStrategy = new OsmoWalletUserFlow(this.manager, this.partnerInvoice, this.ibexService, this.user, this.invoice);
        const response = await osmoUserStrategy.deposit();
        if (response == PartnerStatus.SUCCESS) {
            const bankStrategy = new PartnerWithdraw(this.partnerModel, this.partnerInvoice, this.sendGridService, this.manager, this.user);
