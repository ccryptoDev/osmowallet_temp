...
export class Lightning extends Send implements SendBtc {
    ...
    async sendAutoconvert(data: SendBtcData): Promise<void> {
        ...
        osmoFiatFeeToCredit = new Decimal(
            new Decimal(inflatedSats).times(SATOSHI_TO_BITCOIN_RATIO).times(buyFee).times(btcPrice).toFixed(2),
        );
        ...
        try {
            ...
            await this.manager.transaction(async (entityManager) => {
                transactionGroup = entityManager.create(TransactionGroup, {
                    ...
                    btcPrice: btcPrice,
                    osmoBusiness: osmoBusiness ?? undefined,
            ...
            await this.ibexService.payInvoice(data.payload.address, ibexAccountId, data.payload.amount * 1000, transactionGroup?.id);
