...
export class Lnurl extends Send implements SendBtc {
    ...
    async sendNative(data: SendBtcData): Promise<any> {
        ...
        await this.manager.transaction('SERIALIZABLE', async entityManager => {
            btcWallet = await findAndLockWallet({entityManager: entityManager, coinId: data.payload.coinId,userId: data.authUser.sub})
    ...
    async sendAutoconvert(data: SendBtcData): Promise<any> {
        ...
        let userFiatWallet: Wallet, osmoWalletFee: Wallet, userBtcWallet: Wallet
        await this.manager.transaction('SERIALIZABLE',async entityManager => {
            let buyFee = 0;
            //Find my tier
            ...
            [userFiatWallet, osmoWalletFee, userBtcWallet] = await Promise.all([
