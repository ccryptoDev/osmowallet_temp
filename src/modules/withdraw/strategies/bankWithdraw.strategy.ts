...
export class BankWithdraw implements Withdraw {
    ...
    async withdraw(): Promise<any> {
        ...
        await this.manager.transaction('SERIALIZABLE', async (entityManager) => {
            ...
            records.monthlyAmassedAmount = new Decimal(records.monthlyAmassedAmount)
                .plus(this.data.amount / this.coin.exchangeRate)
                .toNumber();
    ...
    private notify(user: User, transactionGroup: TransactionGroup) {
        ...
        const osmoTemplate = new WithdrawalPendingOsmoTemplate(emails, user.firstName + ' ' + user.lastName, user.email, {
            amount: this.data.amount,
        ...
        const date = new Date();
        const template = new WithdrawalPendingTemplate([{ email: user.email, name: user.firstName }], {
            amount: this.data.amount,
            currency: transactionGroup.transactionCoin.acronym,
