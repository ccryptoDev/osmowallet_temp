...
export class StableWithdraw implements Withdraw {
    ...
    private notify(transactionGroup: TransactionGroup, user: User, amount: number) {
        ...
        const date = new Date();
        const template = new WithdrawalPendingOsmoTemplate(emails, user.firstName + ' ' + user.lastName, user.email, {
            amount: this.body.amount,
        ...
        const confirmationUserTemplate = new WithdrawalPendingTemplate([{ email: user.email, name: user.firstName }], {
            amount: amount,
            currency: this.coin.acronym,
            date: formatDateToSpanish(date),
            status: Status.PENDING,
